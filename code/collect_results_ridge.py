...
covar_name = "CovarPert"
output_name = "OutputPert"
naive_name = "Doubling"
sgd_name = "Doubling-SGD"
...
data_arrays = [opt_datas, opt_betas, covar_datas, covar_betas, output_datas, output_betas, naive_datas, naive_betas] #, sgd_datas, sgd_betas]
...
covar_datas  = np.array(covar_datas)
output_datas = np.array(output_datas)
naive_datas  = np.array(naive_datas)
#sgd_datas    = np.array(sgd_datas)

covar_succs = covar_datas[:,:,SUCC]
output_succs = output_datas[:,:,SUCC]
naive_succs = naive_datas[:,:,SUCC]
#sgd_succs = naive_datas[:,:,SUCC]


print("Fraction of successes:")
for i in range(len(alphas)):
  print(covar_name + " = " + str(sum(covar_succs[i]) / float(len(covar_succs[i]))) + ", " + output_name + " = " + str(sum(output_succs[i]) / float(len(output_succs[i]))) + ", " + naive_name + " = " + str(sum(naive_succs[i]) / float(len(naive_succs[i])))) # + ", " + sgd_name + " = " + str(sum(sgd_succs[i]) / float(len(sgd_succs[i]))))

# ERROR plot
opt_err = np.average([np.average(elist) for elist in opt_datas[:,:,OPT_ERR]])
avg_covar_excess_errs = get_avg(covar_datas[:,:,EXCESS_ERR], covar_succs)
...
#avg_sgd_excess_errs = get_avg(sgd_datas[:,:,EXCESS_ERR], sgd_succs)

plt.figure()
myplot(alphas, [opt_err]*len(alphas), color=opt_color, linewidth=opt_linewidth, label=opt_name)
myplot(alphas, [opt_err + a for a in avg_covar_excess_errs], color=noisered_color, linewidth=linewidth, label=noisered_name)
#myplot(alphas, [opt_err + a for a in avg_output_excess_errs], color=output_color, linewidth=linewidth, label=output_name)
myplot(alphas, [opt_err + a for a in avg_naive_excess_errs], color=naive_color, linewidth=linewidth, label=naive_name)
#myplot(alphas, [opt_err + a for a in avg_sgd_excess_errs], color=sgd_color, linewidth=linewidth, label=sgd_name)
...
covar_90th_excess_errs = get_percentile(0.9, covar_datas[:,:,EXCESS_ERR], covar_succs)
output_90th_excess_errs = get_percentile(0.9, output_datas[:,:,EXCESS_ERR], output_succs)

plt.figure()
myplot(alphas, [opt_err]*len(alphas), color=opt_color, linewidth=opt_linewidth, label=opt_name)
myplot(alphas, [opt_err + a for a in avg_covar_excess_errs], color=noisered_color, linewidth=linewidth, label=noisered_name + " - average")
myplot(alphas, [opt_err + a for a in covar_90th_excess_errs], color=noisered_color, linewidth=opt_linewidth, linestyle="--", label=noisered_name + " - 90th percentile")
#myplot(alphas, [opt_err + a for a in avg_output_excess_errs], color=output_color, linewidth=linewidth, label=output_name + " - average")
#myplot(alphas, [opt_err + a for a in output_90th_excess_errs], color=output_color, linewidth=opt_linewidth, linestyle="--", label=output_name + " - 90th percentile")
myplot(alphas, [opt_err + a for a in alphas], color='black', linewidth=1.0, linestyle="--", label=opt_name + " + $\\alpha$")
...
avg_covar_mses = get_avg(covar_datas[:,:,MSE], covar_succs)
...
myplot(alphas, avg_covar_mses, color=noisered_color, linewidth=linewidth, label=noisered_name)
#myplot(alphas, avg_output_mses, color=output_color, linewidth=linewidth, label=output_name)
...
avg_covar_sv_epss = get_avg(covar_datas[:,:,SV_EPS], covar_succs, lambda x: 2.0*x)  # FACTOR 2 BECAUSE ALGS WERE UNDER-ESTIMATING
avg_covar_nr_epss = get_avg(covar_datas[:,:,NR_EPS], covar_succs)
avg_output_sv_epss = get_avg(output_datas[:,:,SV_EPS], output_succs, lambda x: 2.0*x)
...
covar_theory = [theory.covar_get_epsilon(alpha, n, dim, max_norm) for alpha in alphas]
...
myplot(alphas, covar_theory, color=theory3_color, label=covar_name + " theory")
myplot(alphas, output_theory, color=theory4_color, label=output_name + " theory")
#myplot(alphas, np.add(avg_output_sv_epss, avg_output_nr_epss), color=output_color, linewidth=linewidth, label=output_name)
#myplot(alphas, np.add(avg_naive_sv_epss, avg_naive_nr_epss), color=naive_color, linewidth=linewidth, label=naive_name)
myplot(alphas, np.add(avg_covar_sv_epss, avg_covar_nr_epss), color=noisered_color, linewidth=linewidth, label=noisered_name)
plt.legend()
...
avg_covar_sv_exp_epss  = get_avg(covar_datas[:,:,SV_EPS], covar_succs,myexp)
avg_covar_nr_exp_epss  = get_avg(covar_datas[:,:,NR_EPS], covar_succs,myexp)
...
avg_covar_exp_epss  = get_avg(np.add(covar_datas[:,:,SV_EPS],covar_datas[:,:,NR_EPS]), covar_succs, myexp)
...
myplot(alphas, exp_list(covar_theory), color=theory3_color, label=covar_name + " theory")
myplot(alphas, exp_list(output_theory), color=theory4_color, label=output_name + " theory")
myplot(alphas, avg_covar_exp_epss, color=noisered_color, linewidth=linewidth, label=noisered_name)
...
print("alpha Covar e^covar Naive e^naive")
a = np.add(avg_covar_sv_epss, avg_covar_nr_epss)
b = np.add(avg_naive_sv_epss, avg_naive_nr_epss)
for j,alpha in enumerate(alphas):
  print(alpha, a[j], myexp(a[j]), b[j], myexp(b[j]))
...
# PRIVACY plot 3.5 - no theory, vs SGD
#plt.figure()
#myplot(alphas, np.add(avg_sgd_sv_epss, avg_sgd_nr_epss), color=sgd_color, linewidth=linewidth, label=sgd_name)
#myplot(alphas, np.add(avg_covar_sv_epss, avg_covar_nr_epss), color=noisered_color, linewidth=linewidth, label=noisered_name)
#plt.legend()
#plt.title("Comparison to " + sgd_name)
#plt.xlabel(xaxis_name)
#plt.ylabel(eps_yaxis_name)
...
plt.figure()
myplot(alphas, np.add(avg_covar_sv_epss, avg_covar_nr_epss), color=noisered_color, linewidth=1.0, label=noisered_name)
myplot(alphas, avg_covar_sv_epss, color=sv_eps_color, linewidth=1.0, label="$\epsilon$ due to AboveThreshold")
myplot(alphas, avg_covar_nr_epss, color=nr_eps_color, linewidth=1.0, label="$\epsilon$ due to NoiseReduction")
plt.legend()
...
plt.figure()
myplot(alphas, avg_covar_exp_epss, color=noisered_color, linewidth=1.0, label=noisered_name)
myplot(alphas, avg_covar_sv_exp_epss, color=sv_eps_color, linewidth=1.0, label="$\exp[\epsilon]$ due to AboveThreshold")
myplot(alphas, avg_covar_nr_exp_epss, color=nr_eps_color, linewidth=1.0, label="$\exp[\epsilon]$ due to NoiseReduction")
plt.legend()
plt.title("Breakdown of privacy loss")
plt.xlabel(xaxis_name)
plt.ylabel(eps_yaxis_name)
...
# NUMBER OF STEPS
avg_covar_inds = get_avg(covar_datas[:,:,IND], covar_succs)
avg_output_inds = get_avg(output_datas[:,:,IND], output_succs)
avg_naive_inds = get_avg(naive_datas[:,:,IND], naive_succs)

plt.figure()
myplot(alphas, avg_covar_inds, color=noisered_color, linewidth=linewidth, label=noisered_name)
#myplot(alphas, avg_output_inds, color=output_color, linewidth=linewidth, label=output_name)
...
# NORM OF HYPOTHESES
avg_covar_norms = get_avg(covar_betas, covar_succs, np.linalg.norm)
...
myplot(alphas, avg_covar_norms, color=noisered_color, linewidth=linewidth, label=noisered_name)
#myplot(alphas, avg_output_inds, color=output_color, linewidth=linewidth, label=output_name)
